...
public class AddMemberCommand extends XcraftCommand {

	public AddMemberCommand(XcraftCommandManager cManager, String command, String name, String pattern, String usage, String desc, String permission) {
	...
	public boolean execute(XcraftPluginManager manager, CommandSender sender, String[] args) {
		...
		if (region != null) {
			...
			region.getRegion().getMembers().addPlayer(member.getName());
			try {
				cManager.worldguard.getRegionManager(player.getWorld()).save();
			} catch (ProtectionDatabaseException e) {
				Messenger.severe("ERROR while saving regions. A member was just added!");
				e.printStackTrace();
			}
			sendInfo(sender, Msg.COMMAND_ADDMEMBER_SUCCESSFULL.toString(Replace.NAME(member.getName()), Replace.REGION(region.getRegion().getId())), true);
			return true;
		}
		sendInfo(sender, Msg.COMMAND_ADDMEMBER_FAIL.toString(), true);
		return false;
	}
