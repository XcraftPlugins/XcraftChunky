...
public class RemoveOwnerCommand extends XcraftCommand {

	public RemoveOwnerCommand(XcraftCommandManager cManager, String command, String name, String pattern, String usage, String desc, String permission) {
	...
	public boolean execute(XcraftPluginManager manager, CommandSender sender, String[] args) {
		...
		if (region != null) {
			...
			region.getRegion().getOwners().removePlayer(owner.getName());
			try {
				cManager.worldguard.getRegionManager(player.getWorld()).save();
			} catch (ProtectionDatabaseException e) {
				Messenger.severe("ERROR while saving regions. A owner was just removed!");
				e.printStackTrace();
			}
			sendInfo(sender, Msg.COMMAND_REMOVEOWNER_SUCCESSFULL.toString(Replace.NAME(owner.getName()), Replace.REGION(region.getRegion().getId())), true);
			return true;
		}
		sendInfo(sender, Msg.COMMAND_REMOVEOWNER_FAIL.toString(), true);
		return false;
	}
